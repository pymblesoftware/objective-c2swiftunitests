...
- (IBAction)onPlayButton:(id)sender
{
    if( ! running )
    {
        ...
        targetDate = [playStartTime dateByAddingTimeInterval:boutTime];
...
- (IBAction)onResetButton:(id)sender
{
    ...
    [self onRedClear:nil];
    [self onBlueClear:nil];
}

-(void) updateRedPointImg
{
    if( redPoints > 9)
    {
        [self.redPointMajorImg setImage:[self imageFromNumber:redPoints/10]];
    }
    else
        [self.redPointMajorImg setImage:[self imageFromNumber:11]];
    
    if( redPoints == 0 )
        [self.redPointMinorImg setImage:[self imageFromNumber:11]];                 // 11 is major number for blank.
    else
        [self.redPointMinorImg setImage:[self imageFromNumber:redPoints % 10]];
    
}

- (IBAction)onRedMinus1:(id)sender
{
    if( redPoints > 0 )
        redPoints--;

    [self updateRedPointImg];
}

- (IBAction)onRedPlus1:(id)sender
{

- (IBAction)onBluePlus1:(id)sender
{
    bluePoints++;
    [self updateBluePointImg];
